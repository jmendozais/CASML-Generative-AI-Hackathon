...
def create_tool_calling_agent(
    ...
    model_runnable = pre | model # pipeline pre -> model
...
class LangGraphResponsesAgent(ResponsesAgent):
    def __init__(
        self,
        uc_tool_names: Sequence[str] = ("main_build.casml.*",),
            ...
            if event[0] == "updates":
                for node_data in event[1].values():
                    for item in self._langchain_to_responses(node_data["messages"]):
                        # TODO: Some queries are returning empty outputs
                        if item['type'] == 'function_call_output' and ('output' not in item.keys() or len(item['output']) == 0):
                            continue
...
print("AGENT llm_endpoint_name: ", model_config.get("llm_endpoint_name"))
...
# TODO: Fix schema references
# Import into the notebook and test if it works